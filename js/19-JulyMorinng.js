...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateSvgPatternColors() {
        ...
        // Expanded color mapping to include all colors in your SVGs
        const COLOR_MAPPING = {
            "#bc1937": "primary",
            "#001d45": "secondary",
            "#fff": "tertiary",
            "white": "tertiary",  // Map both "white" and "#fff"
            "#56a578": "secondary", // Add mapping for this color
            "#939598": "tertiary"  // Add mapping for this color
        };
        ...
        model.traverse(child => {
            if (child.isMesh && child.userData.pattern && child.userData.pattern.isSvg) {
                ...
                originalColors.forEach(originalColor => {
                    // Normalize color for comparison (lowercase, remove # if present)
                    ...
                    for (const [color, group] of Object.entries(COLOR_MAPPING)) {
                        ...
                        if (normalizedColor === normalizedMapColor) {
                            colorGroup = group;
                            break;
                        }
                    }
                    ...
                    if (colorGroup) {
                        ...
                        // Convert Three.js Color to hex if needed
                        if (typeof replacementColor === 'number') {
                            replacementColor = '#' + replacementColor.toString(16).padStart(6, '0');
                        } else if (!replacementColor.startsWith('#')) {
                            replacementColor = '#' + replacementColor;
                        }

                        // Create regex that matches the color in various formats
