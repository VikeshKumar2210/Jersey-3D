document.addEventListener("DOMContentLoaded", function () {
    ...
    async function applyImageToSelectedMesh(imageFile, uv) {
        ...
        // Log the mesh area when placing an image
        const meshArea = calculateMeshArea(selectedMesh);
        console.log(`Applying image to mesh "${selectedMesh.name}" with area: ${meshArea.toFixed(2)} square units`);
    ...
    function applyTextToSelectedMesh(text, uv) {
        ...
        // Log the mesh area when placing text
        const meshArea = calculateMeshArea(selectedMesh);
        console.log(`Applying text to mesh "${selectedMesh.name}" with area: ${meshArea.toFixed(2)} square units`);
    ...
    function selectMeshFromButton(meshName, decalType = 'text') {
        ...
        // Calculate and log the mesh area
        const meshArea = calculateMeshArea(selectedMesh);
        console.log(`Mesh "${meshName}" has an area of: ${meshArea.toFixed(2)} square units`);

        const centerUV = calculateMeshCenterUV(selectedMesh);
        ...
        console.log(`Selected ${meshName} (${decalType}) at UV: ${centerUV.x.toFixed(2)}, ${centerUV.y.toFixed(2)}`);

        // Highlight the selected button in the appropriate container
    ...
    function calculateMeshArea(mesh) {
        ...
        // If the geometry has an index (uses triangles)
        if (geometry.index) {
        ...
        // If no index (non-indexed geometry)
        else {
