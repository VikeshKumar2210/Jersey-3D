...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const designColors = {
        halfSleeves: {
            primary: 0xffffff, // Example color for Design1
            secondary: 0xffffff,
            tertiary: 0xffffff,
        },
        fullSleeves: {
    ...
    const modelSvgPatterns = {
        'Green-Shirt-HALF-SLEEVE.glb': {
            Plane: "assets/ModalPatterns/modal2/Front.svg",
            Plane_1: "assets/ModalPatterns/modal2/Back.svg",
            Plane_2: "assets/ModalPatterns/modal2/Neck.svg",
            Plane_3: "assets/ModalPatterns/modal2/Left-Sleeve.svg",
            Plane_4: "assets/ModalPatterns/modal2/Right-Sleeve.svg",
        },
    ...
    function loadSvgPatternForMesh(mesh, svgPath, colorMappings = {}) {
        fetch(svgPath)
            ...
            .then(svgContent => {
                ...
                // Determine the color mapping based on the original colors
                const colorMapping = determineColorMapping(originalColors);
                ...
                mesh.userData.pattern = {
                    ...
                    colorMapping: colorMapping, // Store the determined mapping
                    colorGroup: colorMappings[mesh.name] || 'primary',
                    isFullCoverage: true,
                    baseTexture: null // Will store the base texture
                };
                ...
                // If viewBox exists but no width/height, use viewBox dimensions
                if (svgElement.viewBox.baseVal && !svgElement.hasAttribute('width') && !svgElement.hasAttribute('height')) {
    ...
    function updateMeshTextureForMesh(mesh, options = {}) {
        ...
        if (mesh.userData.pattern && mesh.userData.pattern.baseTexture) {
            ...
            if (!mesh.userData.primaryMaskCanvas) {
                ...
                const primaryColor = (mesh.userData.pattern?.currentPrimaryHex || "#a5172c").toLowerCase();
    ...
    function updateSvgPatternColors() {
        ...
        model.traverse(child => {
            if (child.isMesh && child.userData.pattern && child.userData.pattern.isSvg) {
                ...
                // Get the color mapping for this specific pattern
                const colorMapping = patternData.colorMapping;
                const currentColorMap = {};

                originalColors.forEach(originalColor => {
                    // Find which group this color belongs to
                    const normalizedColor = originalColor.toLowerCase();
                    let colorGroup = null;
                    ...
                    if (colorGroup) {
                        switch (colorGroup) {
                            case "primary":
                                replacementColor = COLORS.primary;
                                break;
                            case "secondary":
                                replacementColor = COLORS.secondary;
                                break;
                            case "tertiary":
                                replacementColor = COLORS.tertiary;
                                break;
                            default:
                                replacementColor = originalColor;
                        }

                        replacementColor = typeof replacementColor === 'number'
                            ? '#' + replacementColor.toString(16).padStart(6, '0')
                            : replacementColor.startsWith('#') ? replacementColor : '#' + replacementColor;
                        ...
                        if (colorGroup) {
                            currentColorMap[colorGroup] = replacementColor.toLowerCase();
                        }
                    }
                });

                child.userData.pattern.currentColorMap = currentColorMap;
                child.userData.pattern.currentPrimaryHex = currentColorMap.primary || originalColors[0];
    ...
    function determineColorMapping(originalColors) {
        // This is your default mapping that works for the first model
        ...
        // This is a mapping that works for your second model example
        ...
        // Check if the colors match the default mapping
        const defaultColorsMatch = originalColors.every(color =>
            Object.keys(defaultMapping).includes(color.toLowerCase()));

        if (defaultColorsMatch) {
            return defaultMapping;
        }

        // Check if the colors match the alternate mapping
        const alternateColorsMatch = originalColors.every(color =>
            Object.keys(alternateMapping).includes(color.toLowerCase()));

        if (alternateColorsMatch) {
            return alternateMapping;
        }

        // If no predefined mapping matches, create a dynamic mapping
        const dynamicMapping = {};

        // Assign primary to the first color, secondary to second, etc.
        originalColors.forEach((color, index) => {
            if (index === 0) dynamicMapping[color] = "primary";
            else if (index === 1) dynamicMapping[color] = "secondary";
            else if (index === 2) dynamicMapping[color] = "tertiary";
            // Ignore additional colors if there are more than 3
        });
    ...
    function loadDefaultModel() {
        ...
        const defaultModelUrl = "assets/models/Green-Shirt-HALF-SLEEVE.glb";
    ...
    function loadModel(url, colorMappings, modelType) {
        ...
        document.querySelectorAll(".meshColorPalette .palette").forEach((palette) => {
            palette.addEventListener("click", (e) => {
                ...
                const selectedMeshGroup = activeMeshRadio.value;

                // Update the color for the selected group
                if (selectedMeshGroup === "Primary") {
                    COLORS.primary = selectedColor;
                } else if (selectedMeshGroup === "Secondary") {
                    COLORS.secondary = selectedColor;
                } else if (selectedMeshGroup === "Tertiary") {
                    COLORS.tertiary = selectedColor;
                }

                // Update all SVG patterns that use these colors
                updateSvgPatternColors();

                // Update the color preview for the selected mesh group
                const colorPreview = document.getElementById(`apply${selectedMeshGroup}`);
                if (colorPreview) {
                    colorPreview.style.backgroundColor = selectedColor;
                }

                // Also update the static preview
                const staticPreview = document.querySelector(`#meshColorpst .${selectedMeshGroup}`);
                if (staticPreview) {
    ...
    const decalControls = document.getElementById("decalControls");
    let activeDecalBounds = null;

    // Add this function to show controls for the active decal
    function showDecalControls(bounds, isText) {
        if (!bounds) return;

        activeDecalBounds = bounds;
        const canvasRect = renderer.domElement.getBoundingClientRect();

        // Clear previous controls
        decalControls.innerHTML = '';

        // Positions for the four corners
        // Positions for the four corners and rotation handle
        const positions = [
            { x: bounds.x, y: bounds.y, class: 'top-left', icon: '↖', action: 'resize' },
            { x: bounds.x + bounds.width, y: bounds.y, class: 'top-right', icon: '↗', action: 'resize' },
            { x: bounds.x, y: bounds.y + bounds.height, class: 'bottom-left', icon: '↙', action: 'lock' },
            { x: bounds.x + bounds.width, y: bounds.y + bounds.height, class: 'bottom-right', icon: '↘', action: 'resize' },
            { x: bounds.x + bounds.width / 2, y: bounds.y - 30, class: 'rotate-handle', icon: '🔄', action: 'rotate' }
        ];

        // Create a control for each corner
        positions.forEach((pos, index) => {
            const control = document.createElement('div');
            control.className = `drag-handle ${pos.class}`;
            control.textContent = pos.icon;
            control.style.position = 'absolute';
            control.style.left = `${pos.x * canvasRect.width}px`;
            control.style.top = `${pos.y * canvasRect.height}px`;
            control.dataset.action = pos.action;

            // Add event listeners based on action
            control.addEventListener('mousedown', (e) => {
                e.stopPropagation();
                handleDecalControlStart(pos.action, e);
            });

            // Assign different functionality to each corner
            switch (index) {
                case 0: // Top-left - rotate
                    control.title = 'Rotate';
                    control.addEventListener('click', () => {
                        console.log('Rotate clicked');
                        // Implement rotate
                    });
                    break;
                case 1: // Top-right - resize
                    control.title = 'Resize';
                    control.addEventListener('click', () => {
                        console.log('Resize clicked');
                        // Implement resize
                    });
                    break;
                case 2: // Bottom-left - lock/unlock
                    control.title = 'Lock/Unlock';
                    control.addEventListener('click', () => {
                        if (isText) {
                            textDecals[activeTextDecalIndex].isLocked = !textDecals[activeTextDecalIndex].isLocked;
                            control.textContent = textDecals[activeTextDecalIndex].isLocked ? "🔓" : "🔒";
                        } else {
                            imageDecals[activeImageDecalIndex].isLocked = !imageDecals[activeImageDecalIndex].isLocked;
                            control.textContent = imageDecals[activeImageDecalIndex].isLocked ? "🔓" : "🔒";
                        }
                        updateMeshTextureWithAllDecals();
                    });
                    break;
                case 3: // Bottom-right - delete
            ...
            decalControls.appendChild(control);
        });

        decalControls.style.display = 'block';
    }
    let isDecalControlActive = false;
    let activeControlAction = null;
    let initialMousePosition = null;
    let initialDecalState = null;

    function handleDecalControlStart(action, event) {
        isDecalControlActive = true;
        ...
        // Store initial decal state
        if (activeTextDecalIndex >= 0) {
            initialDecalState = {
                type: 'text',
                decal: { ...textDecals[activeTextDecalIndex] },
                bounds: calculateDecalBounds(textDecals[activeTextDecalIndex])
            };
        } else if (activeImageDecalIndex >= 0) {
            initialDecalState = {
                type: 'image',
                decal: { ...imageDecals[activeImageDecalIndex] },
                bounds: calculateDecalBounds(imageDecals[activeImageDecalIndex])
            };
        }

        document.addEventListener('mousemove', handleDecalControlMove);
        document.addEventListener('mouseup', handleDecalControlEnd);
    }
    function handleDecalControlEnd() {
        if (isDecalControlActive) {
            saveState(); // Save state after transformation
            isDecalControlActive = false;
            activeControlAction = null;
            initialMousePosition = null;
            initialDecalState = null;

            document.removeEventListener('mousemove', handleDecalControlMove);
            document.removeEventListener('mouseup', handleDecalControlEnd);
        }
    }
    function handleDecalControlMove(event) {
        if (!isDecalControlActive || !initialDecalState) return;

        const deltaX = event.clientX - initialMousePosition.x;
        const deltaY = event.clientY - initialMousePosition.y;

        if (activeControlAction === 'rotate') {
            // Calculate rotation angle based on mouse movement
            const centerX = initialMousePosition.x + (initialDecalState.bounds.width * renderer.domElement.clientWidth) / 2;
            const centerY = initialMousePosition.y + (initialDecalState.bounds.height * renderer.domElement.clientHeight) / 2;

            const angle = Math.atan2(event.clientY - centerY, event.clientX - centerX) -
                Math.atan2(initialMousePosition.y - centerY, initialMousePosition.x - centerX);

            if (initialDecalState.type === 'text') {
                textDecals[activeTextDecalIndex].rotation = initialDecalState.decal.rotation + angle;
                rotateSlider.value = THREE.MathUtils.radToDeg(textDecals[activeTextDecalIndex].rotation);
                rotationValueSpan.textContent = Math.round(THREE.MathUtils.radToDeg(textDecals[activeTextDecalIndex].rotation));
            } else {
                imageDecals[activeImageDecalIndex].rotation = initialDecalState.decal.rotation + angle;
                rotateImgSlider.value = THREE.MathUtils.radToDeg(imageDecals[activeImageDecalIndex].rotation);
                rotateImgValueSpan.textContent = Math.round(THREE.MathUtils.radToDeg(imageDecals[activeImageDecalIndex].rotation));
            }
        }
        else if (activeControlAction === 'resize') {
            // Calculate scale based on mouse movement
            const scaleDelta = 1 + (deltaX + deltaY) / 100; // Adjust sensitivity as needed

            if (initialDecalState.type === 'text') {
                // For text, we change font size
                textDecals[activeTextDecalIndex].fontSize = Math.max(10, Math.min(100,
                    initialDecalState.decal.fontSize * scaleDelta));
                currentFontSize = textDecals[activeTextDecalIndex].fontSize;
            } else {
                // For images, we change scale
                imageDecals[activeImageDecalIndex].scale = Math.max(0.2, Math.min(4,
                    initialDecalState.decal.scale * scaleDelta));
                resizeImgSlider.value = imageDecals[activeImageDecalIndex].scale * 50;
                resizeValueSpan.textContent = Math.round(imageDecals[activeImageDecalIndex].scale * 100);
            }
        }

        updateMeshTextureWithAllDecals();
    }
    // Add this function to hide controls
    function hideDecalControls() {
        decalControls.style.display = "none";
        activeDecalBounds = null;

        // Clean up any active control state
        if (isDecalControlActive) {
            document.removeEventListener('mousemove', handleDecalControlMove);
            document.removeEventListener('mouseup', handleDecalControlEnd);
            isDecalControlActive = false;
    ...
    decalControls.querySelector(".resize").addEventListener("click", () => {
        // Implement resize functionality
        console.log("Resize clicked");
        // You can link this to your existing resize controls
    });

    decalControls.querySelector(".rotate").addEventListener("click", () => {
        // Implement rotate functionality
        console.log("Rotate clicked");
        // You can link this to your existing rotate controls
    });

    decalControls.querySelector(".lock").addEventListener("click", () => {
        if (activeTextDecalIndex >= 0) {
            textDecals[activeTextDecalIndex].isLocked = !textDecals[activeTextDecalIndex].isLocked;
            this.textContent = textDecals[activeTextDecalIndex].isLocked ? "🔓" : "🔒";
            updateMeshTextureWithAllDecals();
        } else if (activeImageDecalIndex >= 0) {
            imageDecals[activeImageDecalIndex].isLocked = !imageDecals[activeImageDecalIndex].isLocked;
            this.textContent = imageDecals[activeImageDecalIndex].isLocked ? "🔓" : "🔒";
            updateMeshTextureWithAllDecals();
        }
    });

    decalControls.querySelector(".remove").addEventListener("click", () => {
    ...
    document.addEventListener('dblclick', (event) => {
        ...
        if (activeTextDecalIndex >= 0) {
            const decal = textDecals[activeTextDecalIndex];
            const bounds = calculateDecalBounds(decal);
            showDecalControls(bounds, true);
        } else if (activeImageDecalIndex >= 0) {
            const decal = imageDecals[activeImageDecalIndex];
            const bounds = {
            ...
            showDecalControls(bounds, false);
        } else {
            hideDecalControls();
        }
    ...
    document.addEventListener("mousedown", (event) => {
        if (isDecalControlActive) return;
        const mouse = getNormalizedMousePosition(event);
        const raycaster = new THREE.Raycaster();
        raycaster.setFromCamera(mouse, camera);
    ...
    function calculateDecalBounds(decal) {
        ...
        if (decal.text) {
            ...
            return {
                x: 0.5 + decal.offset.x - textWidth / canvasWidth / 2,
                y: 0.5 + decal.offset.y - textHeight / canvasHeight / 2,
                width: textWidth / canvasWidth,
                height: textHeight / canvasHeight,
            };
        } else {
