document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateMeshTextureWithAllDecals() {
        ...
        const meshTextDecals = textDecals.filter(
            (decal) => decal.mesh === selectedMesh,
        );
        const meshImageDecals = imageDecals.filter(
            (decal) => decal.mesh === selectedMesh,
        );
        const meshPatternDecals = patternDecals.filter(
            (decal) => decal.mesh === selectedMesh,
    ...
    function applyTextToSelectedMesh(text, uv) {
        ...
        // Show text editing screen (screen 3) whenever text is placed
    ...
    function selectMeshFromButton(meshName) {
        ...
        console.log(
            `Selected mesh from button: ${selectedMesh ? selectedMesh.name : "None"}`,
        );
    }
  ...
  `,
    ...
    deleteButton.addEventListener("click", () => {
        ...
        if (patternToDelete) {
            ...
            patternDecals = patternDecals.filter(
                (d) => d.uuid !== patternToDelete.uuid,
            );
            updateAllMeshTextures();
            return;
        }
        ...
        if (selectedMesh) {
            if (
                textDecals.filter((d) => d.mesh === selectedMesh).length > 0 ||
