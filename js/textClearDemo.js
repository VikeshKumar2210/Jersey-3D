document.addEventListener("DOMContentLoaded", function () {
    ...
    // Replace your current renderer initialization with:
    ...
    function createTextTexture(text, color = selectedTextColor, fontFamily = "Arial", isSelected = false,) {
        ...
        context.font = `bold ${currentFontSize * 4}px ${fontFamily}`; // Scale up font size
            ...
            'Modal2FullSleeves.glb': {
                placementMeshes: {
                    Plane: {
                        displayName: "Front",
                        image: "front.png",
                        targetAngle: 0 // Front view
                    },
                    Plane_1: {
                        displayName: "Back",
                        image: "back.png",
                        targetAngle: Math.PI // Back view
                    },
                    Plane_4: {
                        displayName: "Left Sleeve",
                        image: "sleeve.png",
                        targetAngle: -Math.PI / 2 // Left view
                    },
                    Plane_5: {
                        displayName: "Right Sleeve",
                        image: "sleeve.png",
                        targetAngle: Math.PI / 2 // Right view
                    }
                }
            },
            'abcdef.glb': {
    ...
    function selectMeshFromButton(meshName, decalType = 'text') {
        ...
        // Get the model configuration for this mesh
        const modelConfig = modelMeshConfigs[currentModelType]?.[currentModelFilename];
        if (modelConfig && modelConfig.placementMeshes[meshName]) {
            // Set target rotation based on the mesh's configured angle
            targetRotationY = modelConfig.placementMeshes[meshName].targetAngle || 0;

            // Immediately rotate the model to face this angle
