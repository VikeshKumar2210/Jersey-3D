document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById('centerDecalButton').addEventListener('click', () => {
        if (activeTextDecalIndex >= 0) {
            textDecals[activeTextDecalIndex].offset.set(0, 0);
            updateMeshTextureWithAllDecals();
        } else if (activeImageDecalIndex >= 0) {
            imageDecals[activeImageDecalIndex].offset.set(0, 0);
            updateMeshTextureWithAllDecals();
        }
    });
        ...
        fullSleeves: {
            'rhj.glb': {
                placementMeshes: {
                    Plane: { displayName: "Front", image: "front.png" },
                    Plane_1: { displayName: "Back", image: "back.png" },
                    Plane_2: { displayName: "Left Sleeve", image: "sleeve.png" },
    ...
    // Add event listeners for angle buttons
    document.querySelectorAll('.frontAngle, .backAngle, .leftAngle, .rightAngle').forEach(button => {
        button.addEventListener('click', function () {
            if (this.classList.contains('frontAngle')) {
                targetRotationY = VIEW_ANGLES.front;
            } else if (this.classList.contains('backAngle')) {
                targetRotationY = VIEW_ANGLES.back;
            } else if (this.classList.contains('leftAngle')) {
                targetRotationY = VIEW_ANGLES.left;
            } else if (this.classList.contains('rightAngle')) {
                targetRotationY = VIEW_ANGLES.right;
            }

            // Disable orbit controls temporarily during animation
    ...
    document
        .querySelectorAll('.gradient-palette input[type="checkbox"]')
        .forEach((checkbox) => {
            checkbox.addEventListener("change", function () {
                // Get all checked boxes
                const checkedBoxes = Array.from(
                    document.querySelectorAll(
                        '.gradient-palette input[type="checkbox"]:checked',
                    ),
                );
    ...
    document.addEventListener('dblclick', (event) => {
        if (isTextMoving || isImageMoving) return; // Skip if we're already moving something
            ...
            for (let i = imageDecals.length - 1; i >= 0; i--) {
                ...
                if (uv.x >= bounds.x && uv.x <= bounds.x + bounds.width &&
                    ...
                    if (activeImageDecalIndex === i) {
                        activeImageDecalIndex = -1; // Deselect if already selected
                    } else {
                        activeImageDecalIndex = i;
                        activeTextDecalIndex = -1; // Deselect any text decal
                    }
                    updateMeshTextureWithAllDecals();
                    updateImagePreview();
                    return;
            ...
            for (let i = textDecals.length - 1; i >= 0; i--) {
                ...
                if (uv.x >= bounds.x && uv.x <= bounds.x + bounds.width &&
                    ...
                    if (activeTextDecalIndex === i) {
                        activeTextDecalIndex = -1; // Deselect if already selected
                        document.getElementById('screen3').style.display = 'none';
                        document.getElementById('screen1').style.display = 'block';
                    } else {
                        activeTextDecalIndex = i;
                        activeImageDecalIndex = -1; // Deselect any image decal
    ...
    function updateMeshTextureForMesh(mesh) {
        ...
        meshTextDecals.forEach((decal) => {
            ...
            const textColor = decal.color.startsWith('#') ? decal.color : `#${decal.color}`;
    ...
    // Toggle outline button (optional)
    ...
    function applyTextToSelectedMesh(text, uv) {
        ...
        const newDecal = {
            ...
            hasOutline: false // Initially no outline
        };
    ...
    document.getElementById("applyLogoButton").addEventListener("click", () => {
        if (pendingImageFile) {
            ...
            // Show image placement buttons
            document.getElementById("ImagePlacementsMeshes").style.display = 'grid';
        } else {
