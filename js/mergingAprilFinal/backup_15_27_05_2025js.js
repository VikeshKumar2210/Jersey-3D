document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadModel(url, colorMappings, modelType) {
        ...
        document
            .querySelectorAll(".textColorPalette .palette")
    ...
    function populateMeshButtons(meshNames) {
        const container = document.getElementById("dynamicMeshButtons");
        container.innerHTML = ""; // Clear existing buttons

        // Create a mapping of mesh names to display names and images
        const displayInfo = {
            Plane: { name: "Front", image: "front.png" },
            Plane_1: { name: "Back", image: "back.png" },
            Plane_2: { name: "Left Sleeve", image: "sleeve.png" },
            Plane_3: { name: "Right Sleeve", image: "sleeve.png" },
            Plane_4: { name: "Collar", image: "collar.png" },
            // Add more mappings as needed
        };

        meshNames.forEach(meshName => {
            const info = displayInfo[meshName] || { name: meshName, image: "default.png" };

            const button = document.createElement("button");
            button.className = `${meshName} meshSide meshSideBtn`;
            button.id = `${meshName}Button`;
            button.dataset.mesh = meshName;
            ...
            button.addEventListener("click", function () {
                selectMeshFromButton(meshName);

                // Screen transition (if needed)
                document.getElementById('screen2').style.display = 'none';
                document.getElementById('screen3').style.display = 'block';
            });
    ...
    function selectMeshFromButton(meshName) {
        ...
        // Calculate the true center of the mesh in UV space
        ...
        console.log(`Selected ${meshName} for placement at UV: ${centerUV.x.toFixed(2)}, ${centerUV.y.toFixed(2)}`);

        // Highlight the selected button
        document.querySelectorAll('#dynamicMeshButtons button').forEach(btn => {
