    ...
    function updateMeshTextureForMesh(mesh, options = {}) {
        ...
        if (mesh.userData.gradient && !skipGradientUpdate) {
            ...
            if (!gradientCache.has(cacheKey)) {
                ...
                // Apply gradient only to primary color areas (#a5172c)
                if (mesh.userData.pattern) {
                    // Create mask from primary color areas
                    const primaryColor = "#a5172c";
                    ...
                    // Create mask where primary color exists
                    ...
                    // Apply the gradient through the mask
                    gradientCtx.save();
                    ...
                    // Apply the mask
                    gradientCtx.save();
                    gradientCtx.globalCompositeOperation = "destination-atop";
        ...
        } else if (mesh.userData.gradient && skipGradientUpdate) {
            // Just draw the cached gradient without recomputing
            const gradient = mesh.userData.gradient;
            const cacheKey = `${mesh.uuid}_${gradient.angle}_${gradient.scale}_${gradient.color1}_${gradient.color2}`;
            if (gradientCache.has(cacheKey)) {
                ctx.drawImage(gradientCache.get(cacheKey), 0, 0);
