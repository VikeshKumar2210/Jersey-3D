document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadModel(url, colorMappings, modelType, callback) {
    ...
    function captureState() {
        ...
        // Capture all mesh gradient data
        const meshGradients = {};
        if (model) {
            model.traverse(child => {
                if (child.isMesh && child.userData.gradient) {
                    meshGradients[child.name] = JSON.parse(JSON.stringify(child.userData.gradient));
                }
            });
        }

        // Capture all color mappings
        const colorMappings = {};
        if (model) {
            model.traverse(child => {
                if (child.isMesh && child.material && child.material.color) {
                    colorMappings[child.name] = `#${child.material.color.getHexString()}`;
                }
            });
        }
        ...
        return {
            ...
            patternDecals: patternDecals.map(decal => ({
                image: decal.image?.src || null,
                offset: decal.offset.clone(),
                rotation: decal.rotation,
                scale: decal.scale,
                mesh: decal.mesh?.name || null,
                uuid: decal.uuid,
                isLocked: decal.isLocked,
                isFullCoverage: decal.isFullCoverage,
                opacity: decal.opacity,
                isSVG: decal.isSVG,
                svgContent: decal.svgContent || null
            })),

            // Gradient data
            gradientMeshes: meshGradients,
    ...
    function applyState(state) {
        ...
        try {
            ...
            backgroundColor = state.backgroundColor || 0xeeeeee;
            scene.background = new THREE.Color(backgroundColor);
            ...
            activeImageDecalIndex = state.activeImageDecoalIndex || -1;

            currentPatternScale = state.currentPatternScale || 1.0;
            currentPatternOpacity = state.currentPatternOpacity || 1.0;
            selectedPatternColor = state.selectedPatternColor || "#1c538e";
            selectedPatternImage = state.selectedPatternImage || null;
            selectedPatternIsSVG = state.selectedPatternIsSVG || false;
            selectedPatternParts = state.selectedPatternParts || {};

            // Restore lights
            lightRotation = state.lightSettings?.rotation || 45;
            lightHeight = state.lightSettings?.height || 5;
            lightIntensity = state.lightSettings?.intensity || 1;
            updateLightPosition();
            mainLight.intensity = lightIntensity;

            // Restore camera and controls
            if (state.cameraPosition) camera.position.copy(state.cameraPosition);
            if (state.cameraRotation) camera.rotation.copy(state.cameraRotation);
            if (state.controlsTarget) controls.target.copy(state.controlsTarget);

            // Handle model loading if needed
            const modelLoadPromise = new Promise((resolve) => {
                if (state.model && state.model !== currentModelFilename) {
                    ...
                    fetch(modelPath)
                        .then(response => {
                            if (response.ok) {
                                loadModel(modelPath, state.modelConfig.colorMappings, state.modelConfig.design, resolve);
                            } else {
                                console.warn("Model file not found, loading default");
                                loadDefaultModel(resolve);
                            }
                        })
                        .catch(error => {
            ...
            modelLoadPromise.then(() => {
                // Restore text decals after model is loaded
                textDecals = state.textDecals.map(decalData => {
                    return {
                        ...
                        uuid: decalData.uuid || THREE.MathUtils.generateUUID(),
                        isLocked: decalData.isLocked || false,
                        outlineWidth: decalData.outlineWidth || 0,
                        outlineColor: decalData.outlineColor || "#000000",
                        hasOutline: decalData.hasOutline || false
                    };
                });
                    ...
                    return new Promise((resolve) => {
                        ...
                        img.onload = () => {
                            const decal = {
                                ...
                                offset: new THREE.Vector2(decalData.offset.x, decalData.offset.y),
                                rotation: decalData.rotation,
                                scale: decalData.scale || 1.0,
                                mesh: model ? model.getObjectByName(decalData.mesh) : null,
                                uuid: decalData.uuid || THREE.MathUtils.generateUUID(),
                                isLocked: decalData.isLocked || false,
                                bounds: {
                                    x: decalData.bounds?.x || 0,
                                    y: decalData.bounds?.y || 0,
                                    width: decalData.bounds?.width || 1,
                                    height: decalData.bounds?.height || 1,
                                    originalWidth: decalData.bounds?.originalWidth || 1024,
                                    originalHeight: decalData.bounds?.originalHeight || 1024
                                }
                            };
                ...
                const patternLoadPromises = state.patternDecals.map(decalData => {
                    return new Promise((resolve) => {
                        if (!decalData.image) {
                            // Handle SVG patterns
                            if (decalData.isSVG && decalData.svgContent) {
                                const svgBlob = new Blob([decalData.svgContent], { type: 'image/svg+xml' });
                                ...
                                img.onload = () => {
                                    const decal = {
                                        ...
                                        uuid: decalData.uuid || THREE.MathUtils.generateUUID(),
                                        isLocked: decalData.isLocked || false,
                                        isFullCoverage: decalData.isFullCoverage || false,
                                        opacity: decalData.opacity || 1.0,
                                        isSVG: true,
                                        svgContent: decalData.svgContent
                                    };
                                    patternDecals.push(decal);
                ...
                Promise.all([...imageLoadPromises, ...patternLoadPromises]).then(() => {
                    // Restore mesh gradients
                    if (model && state.gradientMeshes) {
                        model.traverse(child => {
                            if (child.isMesh && state.gradientMeshes[child.name]) {
                                child.userData.gradient = state.gradientMeshes[child.name];
