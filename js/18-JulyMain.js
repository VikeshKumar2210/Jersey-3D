...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const modelSvgPatterns = {
        'Liberty-Jersey.glb': {
            Plane: "assets/ModalPatterns/modal6/Front.svg",
            Shoulder: "assets/ModalPatterns/modal1/Back.svg",
            Plane_4: "assets/ModalPatterns/modal1/AL.svg",
            Plane_5: "assets/ModalPatterns/modal1/AR.svg",
        ...
        fetch(svgPath)
            ...
            .then(svgContent => {
                ...
                mesh.userData.pattern = {
                    ...
                    originalColors: extractColorsFromSVG(svgContent),
    ...
    function updateSvgPatternColors() {
        ...
        model.traverse(child => {
            if (child.isMesh && child.userData.pattern && child.userData.pattern.isSvg) {
                ...
                child.userData.pattern.currentPrimaryHex = currentColorMap.primary || "#a5172c";
    ...
    function showDecalControls(bounds, isText) {
        ...
        // Create the four control buttons as per your HTML structure
        const controls = [
            { class: 'resize', title: 'Resize', icon: '↔', action: 'resize' },
            { class: 'rotate', title: 'Rotate', icon: '↻', action: 'rotate' },
            { class: 'lock', title: 'Lock/Unlock', icon: '🔒', action: 'lock' },
            { class: 'remove', title: 'Remove Decal', icon: '✕', action: 'remove' }
        ];

        // Position the controls around the decal
        const positions = [
            { x: bounds.x, y: bounds.y },                     // top-left (resize)
            { x: bounds.x + bounds.width, y: bounds.y },      // top-right (rotate)
            { x: bounds.x, y: bounds.y + bounds.height },     // bottom-left (lock)
            { x: bounds.x + bounds.width, y: bounds.y + bounds.height } // bottom-right (remove)
        ];

        controls.forEach((control, index) => {
            const pos = positions[index];
            const controlElement = document.createElement('div');
            controlElement.className = `drag-handle ${control.class}`;
            controlElement.title = control.title;
            controlElement.textContent = control.icon;
            controlElement.style.position = 'absolute';
            controlElement.style.left = `${pos.x * canvasRect.width}px`;
            controlElement.style.top = `${pos.y * canvasRect.height}px`;
            controlElement.dataset.action = control.action;

            // Position adjustments to center the handles
            controlElement.style.transform = index % 2 === 0 ?
                'translate(-50%, -50%)' : 'translate(50%, 50%)';

            // Add event listeners
            controlElement.addEventListener('mousedown', (e) => {
                e.stopPropagation();
                handleDecalControlStart(control.action, e);
            });

            // Special click handler for lock and remove
            if (control.action === 'lock' || control.action === 'remove') {
                controlElement.addEventListener('click', (e) => {
                    e.stopPropagation();
                    if (control.action === 'lock') {
                        toggleDecalLock(isText);
                    } else if (control.action === 'remove') {
                        removeActiveDecal(isText);
                    }
                });
            }

            decalControls.appendChild(controlElement);
        });

        decalControls.style.display = 'block';
    }
    function toggleDecalLock(isText) {
        if (isText && activeTextDecalIndex >= 0) {
            textDecals[activeTextDecalIndex].isLocked = !textDecals[activeTextDecalIndex].isLocked;
            const lockBtn = decalControls.querySelector('.lock');
            lockBtn.textContent = textDecals[activeTextDecalIndex].isLocked ? "🔓" : "🔒";
            lockBtn.title = textDecals[activeTextDecalIndex].isLocked ? "Unlock" : "Lock";
        } else if (!isText && activeImageDecalIndex >= 0) {
            imageDecals[activeImageDecalIndex].isLocked = !imageDecals[activeImageDecalIndex].isLocked;
            const lockBtn = decalControls.querySelector('.lock');
            lockBtn.textContent = imageDecals[activeImageDecalIndex].isLocked ? "🔓" : "🔒";
            lockBtn.title = imageDecals[activeImageDecalIndex].isLocked ? "Unlock" : "Lock";
        }
        updateMeshTextureWithAllDecals();
    }

    function removeActiveDecal(isText) {
        if (isText && activeTextDecalIndex >= 0) {
    ...
    function handleDecalControlMove(event) {
        ...
        if (activeControlAction === 'resize') {
            // Calculate scale based on mouse movement
            const scaleDelta = 1 + (deltaX + deltaY) / 100;
        ...
        else if (activeControlAction === 'rotate') {
