document.addEventListener("DOMContentLoaded", function () {
    ...
    async function applyImageToSelectedMesh(imageFile, uv) {
        ...
        try {
            const { texture, bounds, originalImage } =
                await createImageTexture(imageFile);
            ...
            console.log(
                `Added image to ${selectedMesh.name} at position x:${uv.x.toFixed(2)}, y:${uv.y.toFixed(2)}`,
            );
        } catch (error) {
    ...
    function applyTextToSelectedMesh(text, uv) {
        ...
        console.log(`Applying decal at UV: ${uv.x}, ${uv.y} to mesh: ${selectedMesh.name}`);
