...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Stripe selection → preview + store model config
    document.querySelectorAll("#jerseyStripesForm input[type=checkbox]").forEach(input => {
        input.addEventListener("change", () => {
            selectedStripe = input.value;
            console.log("Stripe selected:", selectedStripe);

            // ✅ Update preview div with the image of the selected stripe
            if (input.checked) {
                const figure = input.closest("label").querySelector("figure img");
                if (figure) {
                    const previewDiv = document.querySelector(".showImgAccordingCollar_Style_Stripes");
                    if (previewDiv) {
                        previewDiv.innerHTML = ""; // clear old previews
                        const clone = figure.cloneNode(true); // copy the <img>
                        previewDiv.appendChild(clone);
                    }
                }
            }

            // ✅ Store model config, not load immediately
            if (selectedCollar && selectedStyle && selectedStripe) {
                const config = modelMap[selectedCollar]?.[selectedStyle]?.[selectedStripe];
                if (config) {
                    pendingModelConfig = config;
                    console.log("✅ Model stored, waiting for Load Modal button:", config);
