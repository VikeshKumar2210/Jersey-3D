...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateSvgPatternColors() {
        ...
        const allowedMeshes = {
            ...
            secondary: ["Plane002_2", "Plane002_3", "Plane002", "Plane002_1"],
        ...
        model.traverse(child => {
            ...
            // ✅ Check if mesh is allowed to receive any color updates
            const isPrimary = allowedMeshes.primary.includes(meshName);
            const isSecondary = allowedMeshes.secondary.includes(meshName);
            const isTertiary = allowedMeshes.tertiary.includes(meshName);

            // Skip meshes that aren't mapped to any group
            if (!isPrimary && !isSecondary && !isTertiary) return;
            ...
            originalColors.forEach(originalColor => {
                ...
                // ✅ Apply color only if this mesh belongs to the relevant group
                const groupAllowed =
                    (colorGroup === "primary" && isPrimary) ||
                    (colorGroup === "secondary" && isSecondary) ||
                    (colorGroup === "tertiary" && isTertiary);

                if (colorGroup && groupAllowed) {
    ...
    const cameraPositions = {
        ...
        right: { x: -5, y: 0, z: 0, angle: Math.PI / 2 },
        left: { x: 5, y: 0, z: 0, angle: -Math.PI / 2 }
    };
