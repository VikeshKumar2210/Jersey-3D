document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadModel(url, colorMappings, modelType) {
        ...
        function updateMeshColor(selectedMesh, selectedColor) {
            ...
            if (selectedMesh === 'Primary' || selectedMesh === 'Secondary' || selectedMesh === 'Tertiary') {
        ...
        document.querySelectorAll('.meshColorPalette .palette').forEach(palette => {
            palette.addEventListener('click', (e) => {
                ...
                if (selectedMesh === 'Primary' || selectedMesh === 'Secondary' || selectedMesh === 'Tertiary') {
                    ...
                    if (meshGroup) {
                        meshGroup.forEach(meshName => {
                            const colorPreview = document.getElementById(`apply${meshName}`);
                            if (colorPreview) {
            ...
            ['Primary', 'Secondary', 'Tertiary'].forEach(group => {
                if (meshColorGroups[group] && meshColorGroups[group].length > 0) {
                    ...
                    if (firstMesh && firstMesh.userData.gradient) {
                        ...
                        if (colorPreview) {
                            colorPreview.style.backgroundColor = firstMesh.userData.gradient.color1;
                        }
                    }
                }
            });
    ...
    // Zoom variables
    const zoomSpeed = 0.5; // Adjust zoom speed as needed
    const minZoom = 10;     // Minimum zoom distance (matches your OrbitControls minDistance)
    const maxZoom = 45;    // Maximum zoom distance (matches your OrbitControls maxDistance)

    // Rotation controls
    rotateRightBtn.addEventListener('mousedown', () => {
        isRotatingRight = true;
        controls.autoRotate = false; // Disable OrbitControls auto-rotate if it's enabled
    });
...
`, 0);
    ...
    function animate() {
        ...
        // Handle manual rotation when buttons are pressed
        if (isRotatingRight) {
