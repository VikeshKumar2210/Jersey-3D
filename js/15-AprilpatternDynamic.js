document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadModel(url, colorMappings, modelType) {
        ...
        loader.load(url, function (gltf) {
            ...
            // Create checkboxes for all meshes
            createMeshCheckboxes(model);
    ...
    function createMeshCheckboxes(model) {
        const container = document.querySelector('.childMeshAppend');
        container.innerHTML = ''; // Clear existing content

        // Create a form element to contain the checkboxes
        const form = document.createElement('form');
        form.className = 'grid3 halfSleevesPattern';
        ...
        model.traverse((child) => {
            if (child.isMesh) {
                ...
                label.dataset.part = child.name.toLowerCase();

                const input = document.createElement('input');
                input.type = 'checkbox';
                input.id = child.name.toLowerCase();
                ...
                labelText.textContent = child.name.toUpperCase();
        ...
        nextLabel.className = 'checkbox-button part-button column2 blackBgBtn';
        nextLabel.onclick = handleNextClick;

        const nextInput = document.createElement('input');
        nextInput.type = 'checkbox';
        nextInput.id = 'next';

        const nextCheckmark = document.createElement('span');
        nextCheckmark.className = 'checkmark d-none';

        const nextLabelText = document.createElement('span');
        nextLabelText.className = 'label-text';
        nextLabelText.innerHTML = 'NEXT <img src="images/icons/arrowLeft.png" alt="Arrow Left" />';

        nextLabel.appendChild(nextInput);
        nextLabel.appendChild(nextCheckmark);
        ...
        document.querySelectorAll('.childMeshAppend input[type="checkbox"]').forEach(checkbox => {
    ...
    // Initialize selectedPatternParts when model loads
    ...
    function applyPatternPreview(selectedParts) {
        ...
        img.onload = function () {
            ...
            selectedParts.forEach(part => {
                let meshName = '';
                switch (part) {
                    case 'front': meshName = 'Plane'; break;
                    case 'back': meshName = 'Plane_1'; break;
                    case 'leftSleeve': meshName = 'Plane_4'; break;
                    case 'rightSleeve': meshName = 'Plane_3'; break;
                    case 'collar': meshName = 'Plane_2'; break;
                }
                ...
                ctx.globalAlpha = currentPatternOpacity; // Use current opacity for preview
                ...
                // Create preview decal with current opacity
                // Inside showPatternPreview function:
                const previewDecal = {
                    image: img,
                    ...
                    isFullCoverage: true,
                    isPreview: true,
                    opacity: currentPatternOpacity // Store current opacity
                };
    ...
    function applyPatternToSelectedParts() {
        ...
        // Get which parts are selected
        const selectedParts = Object.keys(selectedPatternParts).filter(part => selectedPatternParts[part]);
        if (selectedParts.length === 0) {
            alert("Please select at least one part (Front, Back, etc.)");
            return;
        }

        // First remove any existing patterns from these parts (if they were previously applied)
        selectedParts.forEach(part => {
            removePatternFromPart(part);
        });
        ...
        // Load the pattern image and apply to selected parts
        const img = new Image();
        img.crossOrigin = "anonymous";

        img.onload = function () {
            // Apply pattern to all selected parts
            selectedParts.forEach(part => {
                let meshName = '';
                switch (part) {
                    case 'front': meshName = 'Plane'; break;
                ...
                const pattern = ctx.createPattern(img, 'repeat');
                ctx.globalAlpha = currentPatternOpacity; // Use current opacity
                ctx.fillStyle = pattern;
                ctx.fillRect(0, 0, canvas.width, canvas.height);
                ctx.globalAlpha = 1.0;
                ...
                // Create permanent pattern decal with current opacity
                // Inside applyPatternToSelectedParts function:
                const newDecal = {
                    image: img,
                    offset: new THREE.Vector2(0, 0),
                    rotation: 0,
    ...
    function removePatternFromPart(part) {
        // Find the mesh that matches the part name (case insensitive)
        let meshName = '';
        model.traverse(child => {
            if (child.isMesh && child.name.toLowerCase() === part.toLowerCase()) {
                meshName = child.name;
            }
        });

        if (!meshName) {
            console.warn(`Mesh not found for part: ${part}`);
            return;
        }
        ...
        console.log(`Removed pattern from ${meshName}`);
    }
    ...
    function updateMeshTextureForMesh(mesh) {
        ...
        // Get ONLY decals that belong to this specific mesh
        ...
        // Draw patterns (full coverage first)
        meshPatternDecals.forEach(decal => {
            ...
            if (decal.isFullCoverage) {
                ...
                // Create a pattern scaling transform
