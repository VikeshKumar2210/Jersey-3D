...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function showDecalControls(bounds, isText) {
        ...
        decalControlHandles = []; // Clear the array
        ...
        const positions = [
            { x: bounds.x, y: bounds.y, class: 'top-left', icon: '↖' },
            { x: bounds.x + bounds.width, y: bounds.y, class: 'top-right', icon: '↗' },
            { x: bounds.x, y: bounds.y + bounds.height, class: 'bottom-left', icon: '↙' },
            { x: bounds.x + bounds.width, y: bounds.y + bounds.height, class: 'bottom-right', icon: '↘' }
        ];
        ...
        positions.forEach((pos, index) => {
            ...
            decalControlHandles.push(control);
    ...
    function updateDecalControlsPosition(bounds) {
        if (!bounds || !decalControlHandles.length) return;

        const canvasRect = renderer.domElement.getBoundingClientRect();

        // Update positions for all control handles
        decalControlHandles[0].style.left = `${bounds.x * canvasRect.width}px`;
        decalControlHandles[0].style.top = `${bounds.y * canvasRect.height}px`;

        decalControlHandles[1].style.left = `${(bounds.x + bounds.width) * canvasRect.width}px`;
        decalControlHandles[1].style.top = `${bounds.y * canvasRect.height}px`;

        decalControlHandles[2].style.left = `${bounds.x * canvasRect.width}px`;
        decalControlHandles[2].style.top = `${(bounds.y + bounds.height) * canvasRect.width}px`;

        decalControlHandles[3].style.left = `${(bounds.x + bounds.width) * canvasRect.width}px`;
        decalControlHandles[3].style.top = `${(bounds.y + bounds.height) * canvasRect.width}px`;
    }
  ...
  `,
    ...
    function animate() {
        ...
        // Update decal controls if active
        if ((activeTextDecalIndex >= 0 || activeImageDecalIndex >= 0) && decalControlHandles.length) {
            const decal = activeTextDecalIndex >= 0 ?
                textDecals[activeTextDecalIndex] :
                imageDecals[activeImageDecalIndex];
            const bounds = calculateDecalBounds(decal);
            updateDecalControlsPosition(bounds);
        }
