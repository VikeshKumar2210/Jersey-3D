        ...
        fetch(svgPath)
            ...
            .then(svgContent => {
                ...
                img.onload = function () {
                    ...
                    // Set canvas dimensions (high resolution for sharpness)
                    const canvas = document.createElement("canvas");
                    const textureSize = 3048; // High resolution to prevent blur
                    canvas.width = textureSize;
                    canvas.height = textureSize / aspectRatio; // Maintain aspect ratio

                    const ctx = canvas.getContext("2d");

                    // Clear canvas with white background (optional)
                    ctx.fillStyle = "#ffffff";
                    ctx.fillRect(0, 0, canvas.width, canvas.height);

                    // Draw SVG to fit the entire canvas
            ...
            .catch(error => {
                ...
                // Fallback to gradient if SVG fails
