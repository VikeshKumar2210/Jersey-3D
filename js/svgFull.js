        ...
        model.traverse((child) => {
            if (child.isMesh) {
                ...
                if (modelPatterns && modelPatterns[child.name]) {
                    // Load SVG pattern with color mappings
    ...
    function loadSvgPatternForMesh(mesh, svgPath, colorMappings = {}) {
        fetch(svgPath)
            ...
            .then(svgContent => {
                ...
                // Get the color group for this mesh (primary, secondary, or tertiary)
                const meshColorGroup = colorMappings[mesh.name] || 'primary';

                // Get the current colors from the COLORS object
                const primaryColor = COLORS.primary;
                const secondaryColor = COLORS.secondary;
                const tertiaryColor = COLORS.tertiary;

                // Replace colors in SVG based on mesh's color group
                let customizedSvg = svgContent;

                // Replace first color with primary, second with secondary, etc.
