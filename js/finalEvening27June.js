document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadSvgPatternForMesh(mesh, svgPath, colorMappings = {}) {
        fetch(svgPath)
            ...
            .catch(error => {
                ...
                const defaultColor = COLORS[meshColorGroup] || "#ffffff";
                mesh.material = new THREE.MeshStandardMaterial({
    ...
    function updateMeshTextureForMesh(mesh) {
        ...
        if (mesh.userData.pattern && mesh.userData.pattern.isSvg) {
            const img = new Image();
            img.onload = function () {
                context.globalAlpha = mesh.userData.pattern.opacity || 1;
                context.drawImage(img, 0, 0, canvas.width, canvas.height);
                context.globalAlpha = 1.0;

                // Continue with other decals
                drawOtherDecals();
            };
        ...
        // 3. Draw SVG pattern if exists
        if (mesh.userData.pattern && mesh.userData.pattern.isSvg && mesh.userData.pattern.svgContent) {
            const img = new Image();
            img.onload = function () {
                ...
                // Continue drawing decals once SVG is loaded
                drawAllDecals();
                applyTexture();
            };
            img.src = URL.createObjectURL(new Blob([mesh.userData.pattern.svgContent], { type: 'image/svg+xml' }));
        } else {
            // If no SVG content or non-SVG, fallback
            drawAllDecals();
            applyTexture();
        }

        function drawAllDecals() {
            // 4. Draw patterns (non-SVG)
            // 3. Draw other pattern decals
