...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const modelSvgPatterns = {
        'Liberty-Jersey.glb': {
            Plane003: "assets/ModalPatterns/modal6/front.svg",
            Plane003_2: "assets/ModalPatterns/modal6/back.svg",
            Plane003_4: "assets/ModalPatterns/modal6/Shoulder.svg",
            Plane003_3: "assets/ModalPatterns/modal6/rightSleeves.svg",
            Plane003_2: "assets/ModalPatterns/modal6/leftSleeve.svg",
            NECK_WHITE: "assets/ModalPatterns/modal6/Collar-Svg.svg",
            Plane001: "assets/ModalPatterns/modal6/Collar-Svg.svg",
            Plane003_5: "assets/ModalPatterns/modal6/Collar-Svg.svg",
            NECK_RED_T: "assets/ModalPatterns/modal6/Collar-Svg.svg",
    ...
    document.getElementById('continueBtnInmodel').addEventListener('click', async () => {
        ...
        for (const meshName in usedMeshes) {
            ...
            console.log("mesh my ,=>>>>>>>>>>>", mesh.name);
            if (mesh.name == 'Torus' && mesh.name == 'Torus001' && mesh.name == 'Torus002' && mesh.name == 'Torus003' && mesh.name == 'Torus004' && mesh.name == 'Torus005' && mesh.name == 'Lace' && mesh.name == 'NECK_WHITE' && mesh.name == 'NECK_RED_T') {
                return;
            }
            ...
            (window.textDecals || []).forEach(decal => {
                ...
                text.setAttribute("font-size", decal.fontSize * 3 || 40);
    ...
    function updateSvgPatternColors() {
        ...
        // Convert Three.js color numbers to hex strings for consistent logging
        ...
        // Log current color settings with proper hex format
        ...
        model.traverse(child => {
            if (child.isMesh && child.userData.pattern && child.userData.pattern.isSvg) {
                ...
                console.log(`Original colors in SVG:`, originalColors);

                const currentColorMap = {};

                originalColors.forEach(originalColor => {
                    // Normalize original color for comparison
                    const normalizedOriginal = originalColor.toLowerCase().replace('#', '');

                    // Find matching color group (if any)
                    let colorGroup = null;
                    for (const [color, group] of Object.entries(COLOR_MAPPING)) {
                        const normalizedMapColor = color.toLowerCase().replace('#', '');
                        if (normalizedOriginal === normalizedMapColor) {
                            colorGroup = group;
                            break;
                        }
                    }
                    ...
                    if (colorGroup) {
                        let replacementColor = COLORS[colorGroup];
                        const originalHex = originalColor.startsWith('#') ? originalColor : `#${originalColor}`;

                        // Convert replacement to proper hex format
                        ...
                        // Create case-insensitive regex for the original color
                        ...
                        // Detailed logging
                        console.log(`- ${colorGroup} color changed from ${originalHex} to ${replacementColor}`);
                    } else {
                        console.log(`- Color ${originalColor} not mapped to any group (kept original)`);
                    }
                });
                ...
                // Rest of your SVG processing code...
                const parser = new DOMParser();
    ...
    function populatePatternForm(meshNames) {
        ...
        // Get the list of meshes to show (default to all if not specified)
        const meshesToShow = modelConfig.patternMeshes || meshNames;

        // Initialize selectedPatternParts with all mesh names set to false
        selectedPatternParts = {};

        meshesToShow.forEach((meshName) => {
            // Skip if this mesh isn't actually in the model
            if (!meshNames.includes(meshName)) return;

            selectedPatternParts[meshName] = false;

            // Get the display name for this mesh
            ...
            'Liberty-Jersey.glb': {
                placementMeshes: {
                    Plane003: {
                        ...
                        imagePosition: { x: 0.5, y: 0.4 }  // Image position
                    },
                    Plane003_1: {
                        displayName: "Back",
                        uvCorrection: { x: 0, y: 0 },
                    ...
                    patternMeshes: ['Plane003', 'Plane003_1', 'Plane003_2', 'Plane003_3',], // Only these will show in pattern form
                    gradientMeshes: ['Plane003', 'Plane003_1', 'Plane003_2', 'Plane003_3',], // Only these will show in pattern form
    ...
    function handleTabChange(activeTab) {
        ...
        switch (activeTab) {
            case 'textTab':
                ...
                const textButton = meshButtonsContainer.querySelector('.Plane003');
                if (textButton) textButton.style.display = 'block';
                break;

            case 'numberTab':
                // Show Plane003_2, Plane003_3, and Plane003_1 (from dynamicMeshButtons)
                const numberButtons = meshButtonsContainer.querySelectorAll('.Plane003_2, .Plane003_3, .Plane003_1');
            ...
            case 'nameTab':
                ...
                const nameButton = meshButtonsContainer.querySelector('.Plane003_1');
                if (nameButton) nameButton.style.display = 'block';
                break;

            case 'logoTab':
                // Show Plane003 and Plane003_4 from dynamicImageMeshButtons
                const logoButtons = imageButtonsContainer.querySelectorAll('.Plane003, .Plane003_4');
    ...
    let activeTab = 'designTab'; // Track current active tab

    // Modify your tab click event listeners to update font size
    document.querySelectorAll('.sidebarTabs .nav-item a').forEach(tab => {
        tab.addEventListener('click', function () {
            const tabId = this.id;
            activeTab = tabId; // Update current active tab

            // Set font size based on active tab
            switch (tabId) {
                case 'textTab':
                    currentFontSize = 40; // Larger font for text tab
                    break;
                case 'nameTab':
                    currentFontSize = 80; // Smaller font for name tab
                    break;
                case 'numberTab':
                    currentFontSize = 200; // Default size for number tab
                    break;
                default:
                    currentFontSize = 60; // Default size for other tabs
            }

            // Update the font size of any active text decal
            if (activeTextDecalIndex >= 0) {
                textDecals[activeTextDecalIndex].fontSize = currentFontSize;
                updateMeshTextureWithAllDecals();
            }

            // Call your existing handleTabChange function if needed
    ...
    function populateGradientForm(meshNames) {
        ...
        // Get the list of meshes to show (default to all if not specified)
        const meshesToShow = modelConfig.gradientMeshes || meshNames;
    ...
    function selectMeshFromButton(meshName, decalType = 'text') {
        ...
        if (decalType === 'text' && isReadyToApplyText && pendingText) {
            ...
            applyTextToSelectedMesh(pendingText, customPosition);
            resetTextPlacementMode();
        }
            ...
            applyImageToSelectedMesh(pendingImageFile, customPosition);
            resetImagePlacementMode();
        }
    }
