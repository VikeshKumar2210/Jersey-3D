document.addEventListener("DOMContentLoaded", function () {
        ...
        model.traverse((child) => {
            if (child.isMesh) {
                ...
                if (modelPatterns && modelPatterns[child.name]) {
                    // First load the SVG to extract its colors
                    fetch(modelPatterns[child.name])
                        .then(response => response.text())
                        .then(svgContent => {
                            // Extract colors from SVG
                            const colors = extractColorsFromSVG(svgContent);

                            // Create default color mappings based on SVG colors
                            const defaultColors = {
                                primary: colors[0] || "#a5172c",
                                secondary: colors[1] || "#231f20",
                                tertiary: colors[2] || "#ffffff"
                            };

                            // Now load the SVG pattern with extracted colors
                            loadSvgPatternForMesh(child, modelPatterns[child.name], defaultColors);

                            // Then update with any custom color mappings if they exist
                            if (colorHex) {
                                setTimeout(() => {
    ...
    function loadSvgPatternForMesh(mesh, svgPath, colorMappings = {}) {
        // Default to original SVG colors if no mappings provided
        const primaryColor = colorMappings.primary || "#a5172c";
        const secondaryColor = colorMappings.secondary || "#231f20";
        const tertiaryColor = colorMappings.tertiary || "#ffffff";
        ...
        fetch(svgPath)
            ...
            .then(svgContent => {
                ...
                // Replace colors in SVG based on provided mappings
                let customizedSvg = svgContent;
                customizedSvg = customizedSvg.replace(/#a5172c/g, primaryColor);
                customizedSvg = customizedSvg.replace(/#231f20/g, secondaryColor);
                customizedSvg = customizedSvg.replace(/#fff/g, tertiaryColor);
                ...
                const img = new Image();
                img.onload = function () {
                    // Calculate mesh dimensions for aspect ratio
                    ...
                    const textureSize = 2048; // High resolution
                    canvas.width = textureSize;
                    canvas.height = textureSize / aspectRatio;
                    ...
                    // Store pattern info with original colors for later updates
                    mesh.userData.pattern = {
                        isSvg: true,
                        svgPath: svgPath,
                        svgContent: svgContent, // Store original for re-coloring
                        originalColors: originalColors,
                        colorGroup: mesh.userData.colorCategory || 'primary'
                    };
            ...
            .catch(error => {
                ...
                const defaultColor = COLORS[mesh.userData.colorCategory || 'primary'] || "#ffffff";
                mesh.material = new THREE.MeshStandardMaterial({
    ...
    function extractColorsFromSVG(svgContent) {
        ...
        // Convert to array and sort by frequency (simple heuristic - assumes most used color is primary)
        const colors = Array.from(colorSet);

        // Return the 3 most common colors (or fill with defaults if less than 3)
        return [
            colors[0] || "#a5172c",  // Primary
            colors[1] || "#231f20",  // Secondary
            colors[2] || "#ffffff"   // Tertiary
        ];
    }
