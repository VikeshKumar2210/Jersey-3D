document.addEventListener("DOMContentLoaded", function () {
    ...
    const modelSvgPatterns = {
        'Modal2FullSleeves.glb': {
            Plane: "assets/ModalPatterns/model1/Front.svg",
            Plane_1: "assets/ModalPatterns/model1/Back.svg",
            Plane_4: "assets/ModalPatterns/model1/AL.svg",
            Plane_5: "assets/ModalPatterns/model1/AR.svg",
    ...
    function loadSvgPatternForMesh(mesh, svgPath, colorMappings = {}) {
        fetch(svgPath)
            ...
            .then(svgContent => {
                ...
                const svgGradientMapping = {
                    "#a5172c": "primary"
                };
                const gradientPrimary = svgGradientMapping["#a5172c"]; // "primary" 

                console.log(`Gradient Primary color: ${gradientPrimary}`);
                // Store which color is primary
                mesh.userData.primaryColor = originalColors.find(color =>
                    svgGradientMapping[color] === "primary"
                );
    ...
    function updateMeshTextureForMesh(mesh) {
        ...
        if (mesh.userData.pattern && mesh.userData.pattern.baseTexture) {
            // Get the base SVG texture image
        ...
        // 3. Draw gradient if it exists (now on top of SVG)
        if (mesh.userData.gradient) {
            ...
            // Set global alpha for the gradient to control its opacity
            context.globalAlpha = 0.7; // Adjust this value (0-1) for desired transparency
            context.fillStyle = canvasGradient;
            context.fillRect(0, 0, canvas.width, canvas.height);
            context.globalAlpha = 1.0; // Reset alpha
        }

        // Rest of your existing code for decals...
        // 4. Draw pattern decals (non-SVG)
    ...
    function updateSvgPatternColors() {
        ...
        // Color mapping configuration - defines which original colors map to which color groups
        const COLOR_MAPPING = {
            "#a5172c": "primary",   // Red -> Primary
            "#231f20": "secondary", // Black -> Secondary
            "#fff": "tertiary"      // White -> Tertiary
        };
        ...
        model.traverse(child => {
            if (child.isMesh && child.userData.pattern && child.userData.pattern.isSvg) {
                ...
                // Log original colors
                console.log(`Original colors in ${patternData.svgPath}:`, originalColors);

                let customizedSvg = patternData.svgContent;
                const newColors = [];

                // Replace each color based on its defined mapping
                originalColors.forEach(originalColor => {
                    ...
                    switch (colorGroup) {
                        ...
                        case "tertiary":
                            ...
                            break;
                        default:
                            replacementColor = originalColor; // Keep original if no mapping
                    }
                    ...
                    newColors.push(replacementColor);
                });

                // Log updated colors
                console.log(`Updated colors for ${child.name}:`, newColors);
                ...
                // Update the texture with dynamic dimensions
