document.addEventListener("DOMContentLoaded", function () {
    ...
    // Add this function to help debug dimensions
    function logDimensions(mesh) {
        if (!mesh) return;

        // 1. Log SVG dimensions (if exists in userData.pattern)
        if (mesh.userData.pattern && mesh.userData.pattern.baseTexture) {
            const svgImg = mesh.userData.pattern.baseTexture.image;
            console.log(`SVG Dimensions for ${mesh.name}:`, {
                width: svgImg.width,
                height: svgImg.height,
                naturalWidth: svgImg.naturalWidth,
                naturalHeight: svgImg.naturalHeight
            });
        }

        // 2. Log mesh dimensions
        const box = new THREE.Box3().setFromObject(mesh);
        const size = new THREE.Vector3();
        box.getSize(size);
        console.log(`Mesh Dimensions for ${mesh.name}:`, {
            width: size.x,
            height: size.y,
            depth: size.z
        });
        ...
        if (mesh.geometry.attributes.uv) {
            ...
            console.log(`UV Dimensions for ${mesh.name}:`, {
                minU: minU,
                maxU: maxU,
                minV: minV,
                maxV: maxV,
    ...
    // Add this debug function to inspect UV coordinates
    function debugUVs(mesh) {
        if (!mesh.geometry.attributes.uv) {
        ...
        console.log(`Mesh ${mesh.name} UV bounds: U(${minU.toFixed(2)}-${maxU.toFixed(2)}), V(${minV.toFixed(2)}-${maxV.toFixed(2)})`);
    }
    ...
    function loadSvgPatternForMesh(mesh, svgPath, colorMappings = {}) {
        fetch(svgPath)
            ...
            .then(svgContent => {
                ...
                // Get SVG dimensions - handle both attributes and viewBox
                ...
                // Analyze UVs to determine scaling needed
                const uvs = mesh.geometry.attributes.uv;
                let minU = Infinity, maxU = -Infinity;
                let minV = Infinity, maxV = -Infinity;
                ...
                // Calculate UV bounds and scale factors
                const uvWidth = maxU - minU;
                const uvHeight = maxV - minV;

                // Create canvas with dimensions matching the UV coverage
                const canvas = document.createElement("canvas");
                canvas.width = Math.ceil(svgWidth * uvWidth);
                canvas.height = Math.ceil(svgHeight * uvHeight);
                ...
                const img = new Image();
                img.onload = function () {
                    // Draw the SVG scaled to match UV coverage
                    ...
                    if (minU !== 0 || minV !== 0) {
                        baseTexture.offset.set(minU, minV);
                    }
                    if (uvWidth !== 1 || uvHeight !== 1) {
                        baseTexture.repeat.set(uvWidth, uvHeight);
                    }
    ...
    function updateMeshTextureForMesh(mesh, options = {}) {
        ...
        const ctx = context;
        // Use UV bounds to determine canvas size
        const uvs = mesh.geometry.attributes.uv;
        ...
        // Calculate dimensions based on UV coverage
        const uvWidth = maxU - minU;
        const uvHeight = maxV - minV;
        canvas.width = Math.ceil(1024 * uvWidth);
        canvas.height = Math.ceil(1024 * uvHeight);
        ...
        if (mesh.userData.pattern && mesh.userData.pattern.baseTexture) {
            ...
            // 💡 Create the primary region mask ONCE after drawing the SVG base
            ...
            // 2. If no SVG pattern, draw white background
            ctx.fillStyle = "#ffffff";
            ctx.fillRect(0, 0, canvas.width, canvas.height);
        }

        // 3. Apply gradient to the primary mask
        if (mesh.userData.gradient && !skipGradientUpdate) {
            ...
            if (!gradientCache.has(cacheKey)) {
                ...
                // 🎯 Use stored primary mask to apply gradient only to primary region
                if (mesh.userData.primaryMaskCanvas) {
                    gradientCtx.save();
        ...
        // 4. Pattern decals — still use stored mask
        ...
        meshPatternDecals.forEach(decal => {
            if (decal.isFullCoverage) {
                ...
                // 🎯 Apply stored mask for primary area
                if (mesh.userData.primaryMaskCanvas) {
                    patternCtx.save();
    ...
    function loadDefaultModel() {
        ...
        const defaultModelUrl = "assets/models/Modal2FullSleeves2.glb";
    ...
    function selectMeshUnderMouse(event) {
        ...
        if (intersects.length > 0) {
            ...
            logDimensions(selectedMesh); // Log dimensions when a mesh is selected
            console.log(
