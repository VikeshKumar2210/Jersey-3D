document.addEventListener("DOMContentLoaded", function () {
    ...
    function showPatternPreview() {
        ...
        // Show/hide color picker based on pattern type
        document.getElementById("svgColorPickerContainer").style.display =
            selectedPatternIsSVG ? "block" : "none";
    ...
    document.getElementById("svgPatternColor").addEventListener("input", function (e) {
        selectedPatternColor = e.target.value;
    });

    document.getElementById("applySvgColor").addEventListener("click", function () {
        if (!selectedPatternIsSVG || !selectedPatternImage) return;

        // Recolor all SVG pattern decals
        patternDecals.forEach(decal => {
            if (decal.isSVG) {
    ...
    function calculateVisualCenterUV(mesh) {
        ...
        // For sleeves and other wrapped parts, adjust Y to be more centered
        if (mesh.name.toLowerCase().includes('sleeve')) {
            uv.y = 0.5; // Force vertical center for sleeves
        }
