...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function applyFabricToZone(zone, fabricKey) {
        if (!model) return;
        const mapConfig = modelFabricMeshMap[currentModelFilename] || {};
        let meshNames = [];

        if (zone.startsWith("Sub")) {
            meshNames = mapConfig.SubZones?.[zone] || [];
        } else {
            meshNames = mapConfig[zone] || [];
        }

        const fabricTex = fabricTextures[fabricKey];
        if (!fabricTex) return;
        ...
        model.traverse((child) => {
            if (child.isMesh && meshNames.includes(child.name)) {
                ...
                // Blend fabric texture on top of base material
                child.material = new THREE.MeshStandardMaterial({
                    map: fabricTex,
                    roughness: 0.9,
                    metalness: 0.2,
    ...
    let selectedFabricZone = null;

    // Radio for Base/Shoulder/Mesh/Subzones
    document.querySelectorAll("#fabricForm input[name='fabric']").forEach((radio) => {
        radio.addEventListener("change", (e) => {
            selectedFabricZone = e.target.value;
            console.log("Fabric zone selected:", selectedFabricZone);
        });
    });
    ...
    document.querySelectorAll("#fabricForm input[name='fabricMaterial[]']").forEach((checkbox) => {
        checkbox.addEventListener("change", (e) => {
            if (!selectedFabricZone) {
                alert("Please select a fabric zone (Base/Shoulder/Mesh/Subzone) first.");
                e.target.checked = false;
                return;
            }

            // Only allow one fabric at a time
            document.querySelectorAll("#fabricForm input[name='fabricMaterial[]']").forEach(cb => cb.checked = false);
            e.target.checked = true;

            const fabricKey = e.target.value;
            console.log(`Applying ${fabricKey} to zone ${selectedFabricZone}`);
            applyFabricToZone(selectedFabricZone, fabricKey);
        });
    });
    const modelFabricMeshMap = {
        "Tri_V-neck_1_stripe_5New3.glb": {
            Base: ["Plane003", "Plane032"], // example meshes
    ...
    const fabricList = {
        fabric1: "images/Patterns/polyester-fabric.jpg",
        fabric2: "images/Patterns/fabric_denim.jpg",
        fabric3: "images/Patterns/fabric4.png",
        // … up to fabric11
    };

    for (const [key, url] of Object.entries(fabricList)) {
        textureLoader.load(url, (tex) => {
            tex.wrapS = tex.wrapT = THREE.RepeatWrapping;
            tex.repeat.set(2, 2);
            tex.anisotropy = renderer.capabilities.getMaxAnisotropy();
            fabricTextures[key] = tex;
        });
    }
