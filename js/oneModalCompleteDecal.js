...
<img src="${meshImageMap[meshName] || 'images/decalPlacements/default.png'}" alt="${displayName}">
    ...
    function updateDecalsListUI() {
        ...
        function createLogosItem(positionName, uuid, type, isText, textContent, imageSrc) {
            ...
            <figure class="m-auto mainLogo">
                ${isText
                    ? `<p style="font-size:14px; font-weight:bold; margin:0;">${textContent}</p>`
        ...
        const showTextDecals = (tabId === "nameTab" || tabId === "numberTab");
        const showImageDecals = (tabId === "main-logo-tab");

        // --- Loop text decals only if Name/Number tab ---
        if (showTextDecals) {
            window.textDecals?.forEach((decal) => {
        ...
        // --- Loop image decals only if Logo tab ---
        if (showImageDecals) {
            window.imageDecals?.forEach((decal) => {
                if (!decal || !decal.uuid) return;
    ...
    document.querySelectorAll('#mainTab .nav-link').forEach(tab => {
        tab.addEventListener('shown.bs.tab', function () {
            updateDecalsListUI(); // refresh decals list when tab changes
        });
    });
