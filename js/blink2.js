...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateSvgPatternColors() {
        ...
        model.traverse(child => {
            ...
            let mergedColorMap = {};
            for (const zone in zoneSvgColorMap) {
                const zoneGroupMap = zoneSvgColorMap[zone][meshName];
                if (zoneGroupMap) {
                    mergedColorMap = { ...mergedColorMap, ...zoneGroupMap };
                }
            }
    ...
    function blinkZoneMeshes(zone) {
        ...
        model.traverse(child => {
            ...
            const hasEmissive = !!child.material.emissive;
            const originalEmissive = hasEmissive ? child.material.emissive.clone() : null;

            // Define blink colors
            const blinkSteps = [
                new THREE.Color(0xffffff), // White
                originalColor,
                new THREE.Color(0x999999), // Grey
                originalColor
            ];
            ...
            const interval = setInterval(() => {
                const newColor = blinkSteps[step];
                child.material.color.copy(newColor);

                if (hasEmissive) {
                    if (step % 2 === 0) {
                        child.material.emissive.set(newColor); // Emissive on blink colors
                    } else {
                        child.material.emissive.set(0x000000); // Off on original
                    }
                }

                step++;
                if (step >= blinkSteps.length) {
            ...
            }, 1000); // 300ms per step (~1.2 sec total)
        });
    }
