document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadSvgPatternForMesh(mesh, svgPath, colorMappings = {}) {
        fetch(svgPath)
            ...
            .then(svgContent => {
                // Store the original SVG content on the mesh
                ...
                // Use the original colors as defaults
                COLORS.primary = originalColors[0];
                COLORS.secondary = originalColors[1];
                COLORS.tertiary = originalColors[2];

                // Get the color group for this mesh
                const meshColorGroup = colorMappings[mesh.name] || 'primary';

                // Replace colors in SVG
                let customizedSvg = svgContent;
                if (originalColors.length >= 1) {
                    customizedSvg = customizedSvg.replace(new RegExp(originalColors[0], 'g'), COLORS.primary);
                }
                if (originalColors.length >= 2) {
                    customizedSvg = customizedSvg.replace(new RegExp(originalColors[1], 'g'), COLORS.secondary);
                }
                if (originalColors.length >= 3) {
                    customizedSvg = customizedSvg.replace(new RegExp(originalColors[2], 'g'), COLORS.tertiary);
                }
                ...
                const img = new Image();
                img.onload = function () {
                    // Calculate mesh dimensions for proper scaling
                    const box = new THREE.Box3().setFromObject(mesh);
                    const size = box.getSize(new THREE.Vector3());

                    // Create canvas that matches the mesh proportions
                    const canvas = document.createElement("canvas");
                    const aspectRatio = size.x / size.y;
                    const baseSize = 2048; // High resolution
                    canvas.width = baseSize;
                    canvas.height = baseSize / aspectRatio;

                    const ctx = canvas.getContext("2d");

                    // Draw SVG to cover entire canvas without repeating
                    ctx.drawImage(img, 0, 0, canvas.width, canvas.height);

                    // Create texture
                    const texture = new THREE.CanvasTexture(canvas);
                    texture.flipY = false;
                    texture.wrapS = THREE.ClampToEdgeWrapping; // Prevent repeating
                    ...
                    mesh.userData.pattern = {
                        ...
                        colorGroup: meshColorGroup,
                        isFullCoverage: true // Mark as full coverage
                    };

                    URL.revokeObjectURL(url);
                };
                img.src = url;
                // Update the mesh texture to include both pattern and any existing decals
    ...
    function updateSvgPatternColors() {
        ...
        model.traverse(child => {
            if (child.isMesh && child.userData.pattern && child.userData.pattern.isSvg) {
                ...
                // Log current colors being applied
                console.log(`Updating colors for ${child.name}:`, {
                ...
                const meshColorGroup = patternData.colorGroup || 'primary';

                // Recolor based on original colors and current palette
                if (originalColors.length >= 1) {
                    customizedSvg = customizedSvg.replace(new RegExp(originalColors[0], 'g'),
                        meshColorGroup === 'primary' ? primaryColor :
                            meshColorGroup === 'secondary' ? secondaryColor : tertiaryColor);
                }
                if (originalColors.length >= 2) {
                    customizedSvg = customizedSvg.replace(new RegExp(originalColors[1], 'g'),
                        meshColorGroup === 'primary' ? secondaryColor :
                            meshColorGroup === 'secondary' ? primaryColor : secondaryColor);
                }
                if (originalColors.length >= 3) {
                    customizedSvg = customizedSvg.replace(new RegExp(originalColors[2], 'g'),
                        meshColorGroup === 'primary' ? tertiaryColor :
                            meshColorGroup === 'secondary' ? tertiaryColor : primaryColor);
                }
                ...
                const img = new Image();
                img.onload = function () {
                    // Calculate the mesh dimensions in UV space
                    ...
                    // Calculate UV width and height
                    const uvWidth = maxU - minU;
                    const uvHeight = maxV - minV;

                    // Determine canvas dimensions based on UV coverage
                    // We'll use a base size (like 1024) and scale proportionally
                    const baseSize = 1024;
                    const canvasWidth = Math.ceil(baseSize * uvWidth);
                    const canvasHeight = Math.ceil(baseSize * uvHeight);

                    // Ensure minimum dimensions
                    const minDimension = 256; // Minimum size to prevent too small textures
                    const canvasWidthFinal = Math.max(canvasWidth, minDimension);
                    const canvasHeightFinal = Math.max(canvasHeight, minDimension);

                    // Create canvas with dynamic dimensions
                    const canvas = document.createElement("canvas");
                    canvas.width = canvasWidthFinal;
                    canvas.height = canvasHeightFinal;
                    const ctx = canvas.getContext("2d");
                    ...
                    // Draw the image, scaling to fit the canvas
                    ctx.drawImage(img, 0, 0, canvas.width, canvas.height);
    ...
    function updateMeshTextureForMesh(mesh) {
        ...
        // 1. First draw the base SVG pattern if it exists
        if (mesh.userData.pattern && mesh.userData.pattern.isSvg) {
            const img = new Image();
            img.onload = function () {
                context.drawImage(img, 0, 0, canvas.width, canvas.height);
                drawOtherElements(); // Proceed to draw other elements
            };
            img.src = URL.createObjectURL(new Blob([mesh.userData.pattern.svgContent], { type: 'image/svg+xml' }));
        } else {
            // 2. If no SVG pattern, draw gradient or solid color background
        ...
        function drawOtherElements() {
            // 3. Draw pattern decals (non-SVG)
