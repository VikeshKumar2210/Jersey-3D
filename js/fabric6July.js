...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Load fabric texture
    textureLoader.load('images/Patterns/fabric_denim.jpg', function (texture) {
        ...
        model.traverse((child) => {
            if (child.isMesh) {
                ...
                child.material = new THREE.MeshStandardMaterial({
                    map: fabricTexture,
                    roughness: 0.7, // Makes it less shiny (more fabric-like)
                    metalness: 0.1, // Slight metallic for some fabric types
                    side: THREE.DoubleSide,
                    transparent: true,
                    alphaTest: 0.5
                });

                // Preserve any existing userData
                if (child.userData) {
                    child.material.userData = child.userData;
                }
            }
        });
    ...
    function updateMeshTextureForMesh(mesh, options = {}) {
        ...
        if (fabricTexture && fabricTexture.image) {
            context.drawImage(fabricTexture.image, 0, 0, canvas.width, canvas.height);
        } else {
    ...
    function loadModel(url, colorMappings, modelType) {
        ...
        loader.load(url, function (gltf) {
            ...
            // Apply fabric texture after model loads
            if (fabricTexture) {
                applyFabricTextureToModel();
            }
