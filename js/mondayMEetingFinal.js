...
document.addEventListener("DOMContentLoaded", function () {
                ...
                originalColors.forEach(originalColor => {
                    ...
                    for (const [color, group] of Object.entries(COLOR_MAPPING)) {
                        ...
                        if (normalizedOriginal === normalizedMapColor) {
                            const replacementColor = COLORS[group];
                            const originalHex = originalColor.startsWith('#') ? originalColor : `#${originalColor}`;
                            const formattedColor = typeof replacementColor === 'number' ?
                            ...
                            customizedSvg = customizedSvg.replace(colorRegex, formattedColor);
                            currentColorMap[group] = formattedColor.toLowerCase();
                            break;
                        }
                    }
                });
    ...
    function loadModel(url, colorMappings, modelType) {
        ...
        // Modify the color palette event listener for tertiary color
        // Replace the existing color palette event listener with this:
        document.querySelectorAll(".meshColorPalette .palette").forEach((palette) => {
            palette.addEventListener("click", (e) => {
                ...
                // Store the current secondary color before any changes
                const currentSecondaryColor = COLORS.secondary;
                ...
                if (selectedMeshGroup === "Primary") {
                    ...
                    // Force update of secondary color patterns
                    updateColorPatternsForGroup('secondary', currentSecondaryColor);
                }
                ...
                const staticPreview = document.querySelector(
                    `#meshColorpst .${selectedMeshGroup}`,
                );
                if (staticPreview) {
        ...
        function updateColorPatternsForGroup(group, color) {
            if (!model) return;
            ...
            model.traverse(child => {
                if (child.isMesh && child.userData.pattern && child.userData.pattern.isSvg) {
                    ...
                    // Check if this mesh's pattern uses the specified color group
                    if (patternData.currentColorMap && patternData.currentColorMap[group]) {
                        // Update the color in the current color map
                        patternData.currentColorMap[group] = color.toLowerCase();

                        // Recreate the SVG with updated colors
                        let customizedSvg = patternData.svgContent;
                        for (const [originalColor, colorGroup] of Object.entries(patternData.originalColorMap || {})) {
                            if (colorGroup === group) {
                                const colorRegex = new RegExp(`(${originalColor}|${originalColor.toLowerCase()}|${originalColor.toUpperCase()})`, 'g');
                                customizedSvg = customizedSvg.replace(colorRegex, color);
                            }
                        }
    ...
    const modelMeshConfigs = {
        ...
        halfSleeves: {
            ...
            'Modal2FullSleeves.glb': {
                ...
                patternMeshes: ['Plane003', 'Plane003_1', 'Plane003_2', 'Plane003_3', 'Plane003_4'], // Only these will show in pattern form
                gradientMeshes: ['Plane003', 'Plane003_1', 'Plane003_2', 'Plane003_3', 'Plane003_4'], // Only these will show in pattern form
            },
