document.addEventListener("DOMContentLoaded", function () {
    ...
    let modelHistory = []; // Array to store model history
    ...
    function loadDefaultModel() {
        ...
        addToHistory({
            design: "Default",
            modal: defaultModelUrl,
            colormappings: defaultColorMappings,
            currentModelType: defaultModelType,
            currentModelFilename: defaultModelUrl.split('/').pop(),
            colors: {
                primary: designColors.halfSleeves.primary,
                secondary: designColors.halfSleeves.secondary,
                tertiary: designColors.halfSleeves.tertiary
            }
        });
    ...
    function loadModel(url, colorMappings, modelType) {
        ...
        function updateMeshColor(selectedMesh, selectedColor) {
            ...
            const currentState = getCurrentColorState();
            ...
            // Add to history after updating
            addToHistory({
                type: 'color-change',
                mesh: selectedMesh,
                color: selectedColor,
                previousState: currentState,
                currentState: getCurrentColorState()
            });
    ...
    document.querySelectorAll(".designsItems img").forEach((img) => {
        img.addEventListener("click", function () {
            ...
            if (modal) {
                // Add to history before loading new model
                addToHistory({
                    design: design,
                    modal: modal,
                    colormappings: colormappings,
                    currentModelType: currentModelType,
                    currentModelFilename: currentModelFilename,
                    colors: {
                        primary: designColors[design]?.primary || COLORS.primary,
                        secondary: designColors[design]?.secondary || COLORS.secondary,
                        tertiary: designColors[design]?.tertiary || COLORS.tertiary
                    }
                });
    ...
    function getCurrentColorState() {
        ...
        // Also capture individual mesh colors if they exist
        if (model) {
            model.traverse(child => {
                if (child.isMesh && child.userData.gradient) {
                    state[child.name] = child.userData.gradient.color1;
                }
            });
        }
    ...
    function addToHistory(state) {
        ...
        if (currentHistoryIndex < modelHistory.length - 1) {
            modelHistory = modelHistory.slice(0, currentHistoryIndex + 1);
        }

        modelHistory.push(state);
        currentHistoryIndex = modelHistory.length - 1;
    ...
    function undo() {
        if (currentHistoryIndex <= 0) return; // Can't undo past first state
    ...
    function redo() {
        if (currentHistoryIndex >= modelHistory.length - 1) return; // Can't redo past last state
    ...
    function restoreFromHistory() {
        const state = modelHistory[currentHistoryIndex];

        if (state.type === 'color-change') {
            // Restore color change
            restoreColorState(state.previousState);
        } else {
            // Restore model change
            // Show loading state
            document.getElementById("preloader").style.display = "flex";
            ...
            if (state.design === "Design1") {
                loadModel(state.modal, design1Mappings, state.design);
            } else if (state.design === "Design2") {
                loadModel(state.modal, design2Mappings, state.design);
            } else {
                loadModel(state.modal, design3Mappings, state.design);
            }

            // Update active design item
            document.querySelectorAll(".designsItems").forEach(item => {
                item.classList.remove("active");
                if (item.querySelector('img').dataset.design === state.design) {
    ...
    function restoreColorState(colorState) {
        // Update global color variables
        COLORS.primary = colorState.primary;
        COLORS.secondary = colorState.secondary;
        COLORS.tertiary = colorState.tertiary;

        // Update all meshes
        if (model) {
            model.traverse(child => {
                if (child.isMesh && child.userData.gradient) {
                    // Check if this mesh has a specific color in the state
                    if (colorState[child.name]) {
                        child.userData.gradient.color1 = colorState[child.name];
                        child.userData.gradient.color2 = colorState[child.name];
                    } else if (meshColorGroups.Primary.includes(child.name)) {
                        child.userData.gradient.color1 = colorState.primary;
                        child.userData.gradient.color2 = colorState.primary;
                    } else if (meshColorGroups.Secondary.includes(child.name)) {
                        child.userData.gradient.color1 = colorState.secondary;
                        child.userData.gradient.color2 = colorState.secondary;
                    } else if (meshColorGroups.Tertiary.includes(child.name)) {
                        child.userData.gradient.color1 = colorState.tertiary;
                        child.userData.gradient.color2 = colorState.tertiary;
                    }
        ...
        // Update UI previews
        updateColorPreviews(colorState);
    }

    function updateColorPreviews(colorState) {
        // Update primary preview
        const colorPreviewPrimary = document.getElementById("applyPrimary");
        if (colorPreviewPrimary) {
            colorPreviewPrimary.style.backgroundColor = colorState.primary;
        }
        const staticPrimary = document.querySelector("#meshColorpst .Primary");
        if (staticPrimary) {
            staticPrimary.style.backgroundColor = colorState.primary;
        }

        // Update secondary preview
        const colorPreviewSecondary = document.getElementById("applySecondary");
        if (colorPreviewSecondary) {
            colorPreviewSecondary.style.backgroundColor = colorState.secondary;
        }
        const staticSecondary = document.querySelector("#meshColorpst .Secondary");
        if (staticSecondary) {
            staticSecondary.style.backgroundColor = colorState.secondary;
        }

        // Update tertiary preview
        const colorPreviewTertiary = document.getElementById("applyTertiary");
        if (colorPreviewTertiary) {
            colorPreviewTertiary.style.backgroundColor = colorState.tertiary;
        }
        const staticTertiary = document.querySelector("#meshColorpst .Tertiary");
        if (staticTertiary) {
            staticTertiary.style.backgroundColor = colorState.tertiary;
        }

        // Update individual mesh previews
        for (const [meshName, color] of Object.entries(colorState)) {
            if (!['primary', 'secondary', 'tertiary'].includes(meshName.toLowerCase())) {
                const preview = document.getElementById(`apply${meshName}`);
                if (preview) {
    ...
    function updateUndoRedoButtons() {
        ...
        if (redoBtn) redoBtn.disabled = currentHistoryIndex >= modelHistory.length - 1;
    }
