document.addEventListener("DOMContentLoaded", function () {
    ...
    // Outline width controls for image decals
    document.querySelector('.outline-minus-image').addEventListener('click', () => {
        if (activeImageDecalIndex >= 0) {
            const decal = imageDecals[activeImageDecalIndex];
            decal.outlineWidth = Math.max(0, decal.outlineWidth - 1);
            document.getElementById('outlineWidthValueImage').textContent = `${decal.outlineWidth}px`;
            updateMeshTextureWithAllDecals();
        }
    });
    document.querySelector('.outline-plus-image').addEventListener('click', () => {
        if (activeImageDecalIndex >= 0) {
            const decal = imageDecals[activeImageDecalIndex];
    ...
    // Outline color palette for image decals
    document.querySelectorAll('.outlineColorImage .palette').forEach(palette => {
        palette.addEventListener('click', (e) => {
            if (activeImageDecalIndex >= 0) {
                const decal = imageDecals[activeImageDecalIndex];
