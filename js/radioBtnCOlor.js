...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelectorAll('input[name="zoneRadio"]').forEach((radio) => {
        radio.addEventListener("change", () => {
            ...
            if (zoneMeshMap[selected]) {
                activeZone = selected;
                console.log(`✅ Selected Zone: ${activeZone}`);
                blinkZoneMeshes(activeZone);
                updateSvgPatternColors();
            } else if (subZoneColorGroupMap[selected]) {
                console.log('subZoneColorGroupMap ===>', subZoneColorGroupMap);
                console.log('selected ===>', selected);
                console.log(`🎯 Selected Subzone: ${selected}`);
                blinkSubzoneMeshes(selected); // ✅ required
                applySubzoneColor(selected);
            }
        });
    });
    ...
    function loadModel(url, colorMappings, modelType) {
        ...
        document.querySelectorAll(".meshColorPalette .colorItems .palette").forEach((palette) => {
            console.log('palette123123213 =>>>>', palette);
            palette.addEventListener("click", (e) => {
