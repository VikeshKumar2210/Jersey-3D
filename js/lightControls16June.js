document.addEventListener("DOMContentLoaded", function () {
    ...
    // from here light
    // Replace with this simpler lighting setup
    const ambientLight = new THREE.AmbientLight(0x404040); // Soft white ambient light
    scene.add(ambientLight);

    // Main directional light that we'll rotate
    ...
    function loadModel(url, colorMappings, modelType) {
        ...
        loader.load(url, function (gltf) {
            ...
            let fv = 4 * Math.atan(maxDim / (1 * cameraDistance)) * (180 / Math.PI);
            camera.fov = fv + cameraDistance - 38;
